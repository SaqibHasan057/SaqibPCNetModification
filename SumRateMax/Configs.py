...
class TopConfig:
    def __init__(self, argv=None):
        ...
        self.function = "Train"
        # communication
        self.user_num = 5  # number of users, currently, if it is set to not 20 or 10, the node_nums have also been reset.
        self.tx_snr = 10
        ...
        self.max_average_power = np.array([0.2 for i in range(self.user_num)])
        self.power_constraint_loss_scale = 100
        ...
        self.training_sample_num = 10000
        self.test_sample_num = 1000  # number of testing  samples
        self.training_mini_batch_size = 100  # training minibach size

        self.test_mini_batch_size = self.test_sample_num
        self.epoch_num = int(1e4)  # number of training epochs
    ...
    def gen_base_folder(self):
        ...
        self.base_folder = format('./SavedModels/UserNum%d_TxSNR%.1fdB_%s/FCN(%s)_MiniBatch%d_Epoch%d' % (self.user_num, self.tx_snr,self.csi_distribution, node_num_str,self.training_mini_batch_size, self.epoch_num))
