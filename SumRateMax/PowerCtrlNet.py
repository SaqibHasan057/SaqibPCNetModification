...
class PowerCtrlNet:
    ...
    def power_constraint(self, power):
        ...
        #power_mean = tf.Print(power_mean,[power_mean])
    ...
    def train_network(self, model_id):
        ...
        loss = tf.negative(sum_rate_dnn) #+ self.top_config.power_constraint_loss_scale*self.power_constraint(self.net_out)
        ...
        while epoch < self.top_config.epoch_num:
            ...
            if epoch % self.top_config.check_interval == 0 or epoch == self.top_config.epoch_num:
                ...
                if sum_rate_dnn_v_binary > max_sum_rate_dnn_v_binary:
                    ...
                    self.assign_best_paras(sess)
                    last_update=epoch
                if((epoch-last_update)>1000):
                    max_sum_rate_dnn_v_binary = sum_rate_dnn_v_binary
                    max_sum_rate_dnn_v = sum_rate_dnn_v
                    self.assign_best_paras(sess)
                    last_update=epoch
                print("Epoch:",epoch,"Sum Rate:",sum_rate_dnn_v,"Sum Rate Binary:",sum_rate_dnn_v_binary,"Max Sum Rate:",max_sum_rate_dnn_v,"Max Sum Rate Binary:",max_sum_rate_dnn_v_binary,"Last Update Step:",epoch-last_update)
