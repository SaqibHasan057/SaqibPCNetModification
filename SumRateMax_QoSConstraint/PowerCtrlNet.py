...
class PowerCtrlNet:
    def __init__(self, top_config):
        ...
        self.moving_power_expectation = None
        self.expectation_momentum=None
        self.expectation_momentum_mask=None
    ...
    def build_network(self, model_id, built_for_training=False):
        ...
        self.moving_power_expectation = tf.Variable(tf.zeros([self.top_config.user_num,1]), dtype=tf.float32,trainable=False)
        self.expectation_momentum = tf.Variable(np.array([0.1]),dtype=tf.float32,trainable=False)
        self.expectation_momentum_mask = tf.Variable(np.array([0.9]),dtype=tf.float32,trainable=False)
    ...
    def power_constraint(self, power):
        power_vector = tf.reshape(power, [-1, self.top_config.user_num, 1])
        power_mean = tf.reduce_mean(power_vector,axis=0)
        #self.moving_power_expectation = self.moving_power_expectation * 0.5 + 0.5 *power_mean
        #x = tf.assign(self.moving_power_expectation,self.expectation_momentum*self.moving_power_expectation + self.expectation_momentum_mask*power_mean)
        loss = tf.reduce_mean(tf.reduce_sum(tf.nn.relu(power_mean -self.top_config.max_average_power), axis=1))
    ...
    def train_network(self, model_id):
        print(self.top_config.max_average_power)
        ...
        loss = tf.negative(sum_rate_dnn) + self.top_config.constraint_loss_scale * self.constraint_loss(self.net_out,self.net_in,self.top_config.noise_power) + self.top_config.power_constraint_loss_scale*self.power_constraint(self.net_out)
        ...
        while epoch < self.top_config.epoch_num:
            ...
            if epoch % self.top_config.check_interval == 0 or epoch == self.top_config.epoch_num:
                ...
                #average_power = sess.run(self.moving_power_expectation, feed_dict={self.is_in_train: False, self.net_in_train:np.zeros([1,self.top_config.input_len],np.float32)})
    ...
    def test_network_online(self, data_io, per_user_rate, sess_in):
        if self.top_config.load_data_from_files:
            per_user_rate_v = sess_in.run(per_user_rate, feed_dict={self.is_in_train: False, self.net_in_train:np.zeros([1,self.top_config.input_len],np.float32)})
        else:
